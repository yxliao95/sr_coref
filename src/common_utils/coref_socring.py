...
from common_utils.coref_utils import (
    ConllToken,
    auto_append_value_to_list,
    auto_extend_value_to_list,
)
...
def align_to_spacy(config, model_cfg, df_spacy, df_pred):
    ...
    df_aligned = (
        df_spacy.merge(df_pred, how="outer", left_index=True, right_on=coref_col_cfg.spacy_index)
        .reset_index()
        .drop(columns=["index"])
    )
    ...
    for _idx, _itemSeries in df_aligned.iterrows():
        ...
        try:
            ...
            if spacy_tokenStr == coref_tokenStr:
                ...
                coref_index_appearance_count_dict[coref_index] = (
                    coref_index_appearance_count_dict.get(coref_index, 0) + 1
                )
                ...
                if coref_tokenStr in spacy_tokenStr and not spacy_tokenStr_isnan and not coref_tokenStr_isnan:
                    if not one2many_flag:
                        logger.debug(
                            "%s) One2many token start: %s | %s", _idx, spacy_tokenStr.encode(), coref_tokenStr.encode()
                        )
                        auto_append_value_to_list(model2spacy_tok_indices, spacy_index, coref_index)
                        coref_index_appearance_count_dict[coref_index] = (
                            coref_index_appearance_count_dict.get(coref_index, 0) + 1
                        ...
                        logger.debug(
                            "%s) One2many token: %s | %s", _idx, spacy_tokenStr.encode(), coref_tokenStr.encode()
                        )
                        auto_append_value_to_list(model2spacy_tok_indices, spacy_index, coref_index)
                        coref_index_appearance_count_dict[coref_index] = (
                            coref_index_appearance_count_dict.get(coref_index, 0) + 1
                ...
                # s.    .   (One2many)
                    ...
                    logger.debug(
                        "%s) Many2one token start: %s | %s", _idx, spacy_tokenStr.encode(), coref_tokenStr.encode()
                    )
                    auto_append_value_to_list(model2spacy_tok_indices, spacy_index, coref_index)
                    coref_index_appearance_count_dict[coref_index] = (
                        coref_index_appearance_count_dict.get(coref_index, 0) + 1
                ...
                # Many2one: TokB,TokC of TokA+TokB+TokC
                    ...
                    coref_index_appearance_count_dict[prev_coref_index] = (
                        coref_index_appearance_count_dict.get(prev_coref_index, 0) + 1
                    )
                    left_str += spacy_tokenStr
                ...
                # elif isnan(coref_index) and spacy_tokenStr not in prev_coref_tokenStr and not many2many_token_flag:
                    ...
                    logger.debug(
                        "%s) Empty coref token: %s | %s", _idx, spacy_tokenStr.encode(), coref_tokenStr.encode()
                    )
                    one2many_flag, many2one_flag, many2many_flag = False, False, False
                    left_str, right_str = "", ""
                ...
                # .     NaN     |                   |   .       NaN     (many2many)
                    ...
                    logger.debug(
                        "%s) Change to many2many token : %s | %s",
                        _idx,
                        spacy_tokenStr.encode(),
                        coref_tokenStr.encode(),
                    )
                    many2many_flag = True
                    ...
                    # Just simply create a spacy tokenObj
                        ...
                        logger.debug(
                            "%s) Unequal token: %s | %s", _idx, spacy_tokenStr.encode(), coref_tokenStr.encode()
                        )
                        checkpoint_token_index = _idx
                        auto_append_value_to_list(model2spacy_tok_indices, spacy_index, coref_index)
                        coref_index_appearance_count_dict[coref_index] = (
                ...
                if True in [one2many_flag, many2one_flag, many2many_flag] and left_str.replace(
                    " ", ""
                ) == right_str.replace(" ", ""):
...
def align_spacy_to_ground_truth(
    gt_token_list, spacy_toekn_list, df_spacy=None, spacy_cfg=None, df_pred=None, model_cfg=None
) -> list[list[int]]:
    """Return:
    ...
    """
    empty_token_idx_with_conll_label_dict: dict[
        int, str
    ] = {}  # e.g. {1110: "(252"} means that token at `1110` has conll label `(252`
    ...
    while curr_gt_token_pointer < len(gt_token_list) and curr_spacy_token_pointer < len(spacy_toekn_list):
        ...
        if curr_spacy_token.strip() == "" and curr_gt_token.strip() == "":
            logger.debug(
                "%s,%s) curr_gt_token, curr_spacy_token are empty: %s | %s",
                curr_gt_token_pointer,
                curr_spacy_token_pointer,
                curr_gt_token,
                curr_spacy_token,
            )
            ...
            if curr_spacy_token.strip() == "":
                logger.debug(
                    "%s,%s) curr_spacy_token is empty in csv: %s",
                    curr_gt_token_pointer,
                    curr_spacy_token_pointer,
                    curr_spacy_token.encode(),
                )
                if df_pred is not None:
                    ...
                    conll_label_str = str(
                        df_pred.loc[curr_spacy_token_pointer, model_cfg.target_column.coref_group_conll]
                    )
            ...
            if curr_gt_token.strip() in ["", "nan"]:
                logger.debug(
                    "%s,%s) curr_gt_token is empty: %s",
                    curr_gt_token_pointer,
                    curr_spacy_token_pointer,
                    curr_gt_token.encode(),
                )
                curr_gt_token_pointer += 1
                continue
        ...
        if curr_gt_token == curr_spacy_token:
            logger.debug(
                "%s,%s) curr_gt_token, curr_spacy_token: %s | %s",
                curr_gt_token_pointer,
                curr_spacy_token_pointer,
                curr_gt_token,
                curr_spacy_token,
            ...
            if curr_spacy_token in curr_gt_token:
                logger.debug(
                    "%s,%s) curr_gt_token contains curr_spacy_token: %s | %s",
                    curr_gt_token_pointer,
                    curr_spacy_token_pointer,
                    curr_gt_token,
                    curr_spacy_token,
                ...
                if df_spacy is not None:
                    ...
                    if curr_gt_token == curr_spacy_token:
                        logger.debug(
                            "%s,%s) [Using spacy token] curr_gt_token, curr_spacy_token: %s | %s",
                            curr_gt_token_pointer,
                            curr_spacy_token_pointer,
                            curr_gt_token,
                            curr_spacy_token,
                        ...
                        logger.debug(
                            "%s,%s) [Using spacy token] curr_gt_token contains curr_spacy_token: %s | %s",
                            curr_gt_token_pointer,
                            curr_spacy_token_pointer,
                            curr_gt_token,
                            curr_spacy_token,
            ...
            if curr_gt_token == right_str:
                logger.debug(
                    "%s,%s) curr_gt_token, right_str: %s | %s",
                    curr_gt_token_pointer,
                    curr_spacy_token_pointer,
                    curr_gt_token,
                    right_str,
...
def convert_non_spacy_token_csv_to_conll_format(
    config,
    model_cfg,
    section_name,
    doc_id,
    coref_index_appearance_count_dict: dict[int, int],
    model2spacy_tok_indices: list[list[int]],
    spacy2gt_tok_indices: list[list[int]],
    df_pred: pd.DataFrame,
    df_spacy: pd.DataFrame,
) -> list[list[ConllToken]]:
    """Args:
    ...
    for spacy_indices in spacy2gt_tok_indices:
        ...
        for model_index in set(model2gt_curr_tok_indices):
            ...
            if conll_label_str != "-1":
                if first_time_appear:
                    ...
                    if (
                        count_coref_index_appear_at_gt_dict[model_index]
                        == coref_index_appearance_count_dict[model_index]
                    ):
        ...
        if len(gt_map_coref_tokens) > 1:
            aggregrated_conll_labels_for_one_row: list[str] = remove_duplicated_conll_label2(
                conll_labels_from_single_or_multi_row
            )
...
def convert_spacy_token_csv_to_conll_format(
    config,
    model_cfg,
    section_name,
    doc_id,
    ...
    target_token_index_and_conll_label_dict,
) -> list[list[ConllToken]]:
    """
    Return:
        list[list[ConllToken]]: The first dimension is sentences. The second dimension is tokens.
    ...
    for indices in spacy2gt_tok_indices:
        ...
        for index in indices:
            ...
            if conll_label_str != "-1" and extra_conll_label != "":
                conll_labels_from_single_or_multi_row.extend(
                    merge_conll_label(ast.literal_eval(conll_label_str), extra_conll_label)
                )
...
def remove_duplicated_conll_label(label_list: list[str]) -> list[str]:
    ...
    new_label_list.extend(
        [
            labelStr
            for labelStr in label_list
            for numberStr in numberStr_set.difference(added_numberStr_set)
            if int(numberStr) == int(labelStr.replace("(", "").replace(")", ""))
        ]
    )
...
def batch_processing(config, model_cfg, section_name, file_name, input_dir, reportInfo: ReportInfo) -> ReportInfo:
    ...
    try:
        ...
        df_gt.iloc[0] = df_gt.iloc[0].apply(
            lambda x: x.replace("\ufeff", "").replace("\xef\xbb\xbf", "") if isinstance(x, str) else x
        )
        df_pred.iloc[0] = df_pred.iloc[0].apply(
            lambda x: x.replace("\ufeff", "").replace("\xef\xbb\xbf", "") if isinstance(x, str) else x
        )
        df_spacy.iloc[0] = df_spacy.iloc[0].apply(
            lambda x: x.replace("\ufeff", "").replace("\xef\xbb\xbf", "") if isinstance(x, str) else x
        )
        ...
        if model_cfg.align_to_spacy:
            ...
            model2spacy_tok_indices, coref_index_appearance_count_dict = align_to_spacy(
                config, model_cfg, df_spacy, df_pred
            )
            ...
            sentence_list_pred = convert_non_spacy_token_csv_to_conll_format(
                config,
                model_cfg,
                section_name,
                doc_id,
                coref_index_appearance_count_dict,
                model2spacy_tok_indices,
                spacy2gt_tok_indices,
                df_pred,
                df_spacy,
            )
        else:
            ...
            target_token_index_and_conll_label_dict = find_cloest_index(
                spacy2gt_tok_indices, empty_token_idx_with_conll_label_dict
            )
            sentence_list_pred = convert_spacy_token_csv_to_conll_format(
                config,
                model_cfg,
                section_name,
                doc_id,
        ...
        temp_dir_for_conll_pred = os.path.join(
            config.output.base_dir, model_cfg.name, config.output.base_temp_dir_name, "predict", section_name
        )
        ...
        temp_gt_dir = os.path.join(
            config.output.base_dir, model_cfg.name, config.output.base_temp_dir_name, "ground_truth", section_name
        )
        ...
        for scorer_metric in scorer_cfg.metrics:
            ...
            if err:
                ...
                (
                    ...
                    coref_precision,
                    coref_f1,
                ) = resolve_conll_script_output(out)
    ...
    for model_cfg in model_cfg_list:
        ...
        for section_name in config.input.section:
            ...
            df_scoring = pd.DataFrame(
                columns=[
                    scoring_col_cfg.doc_id,
                    scoring_col_cfg.conll_f1_avg,
                    scoring_col_cfg.conll_recall_avg,
                    scoring_col_cfg.conll_precision_avg,
                    scoring_col_cfg.muc_f1,
                    scoring_col_cfg.muc_recall,
                    scoring_col_cfg.muc_precision,
                    scoring_col_cfg.bcub_f1,
                    scoring_col_cfg.bcub_recall,
                    scoring_col_cfg.bcub_precision,
                    scoring_col_cfg.ceafe_f1,
                    scoring_col_cfg.ceafe_recall,
                    scoring_col_cfg.ceafe_precision,
                    scoring_col_cfg.mention_f1,
                    scoring_col_cfg.mention_recall,
                    scoring_col_cfg.mention_precision,
                ]
            )
            ...
            with ProcessPoolExecutor(max_workers=config.thread.workers) as executor:
                for file_name in FILE_CHECKER.filter(os.listdir(input_dir)):
                    ...
                    tasks.append(
                        executor.submit(
                            batch_processing, config, model_cfg, section_name, file_name, input_dir, reportInfo
                        )
                    )
                ...
                for future in tqdm(as_completed(tasks), total=len(tasks)):
                    ...
                    df_scoring.loc[len(df_scoring)] = [
                        reportInfo.doc_id,
                        reportInfo.conll_f1_avg,
                        reportInfo.conll_recall_avg,
                        reportInfo.conll_precision_avg,
                        reportInfo.coref_f1_dict["muc"],
                        reportInfo.coref_recall_dict["muc"],
                        reportInfo.coref_precision_dict["muc"],
                        reportInfo.coref_f1_dict["bcub"],
                        reportInfo.coref_recall_dict["bcub"],
                        reportInfo.coref_precision_dict["bcub"],
                        reportInfo.coref_f1_dict["ceafe"],
                        reportInfo.coref_recall_dict["ceafe"],
                        reportInfo.coref_precision_dict["ceafe"],
                        reportInfo.mention_f1_dict["muc"],
                        reportInfo.mention_recall_dict["muc"],
                        reportInfo.mention_precision_dict["muc"],
                    ]
