...
from common_utils.nlp_utils import (
    align_byIndex_individually_nestedgruop,
    align_coref_groups_in_conll_format,
)
...
def init_coref_model(config, model_dir=None, doc_encoder_dir=None):
    """Initial the model

    Args:
        config (Hydra config): By default using this config to get the `model_dir` and `doc_encoder_dir`.
            Must include the following: `config.fastcoref_joint.model_dir` and `config.fastcoref_joint.doc_encoder_dir`.
            Could be set to None if the next two args are provided.
        model_dir (str, optional): If provided, not using the default `config.fastcoref_joint.model_dir`. Defaults to None.
        doc_encoder_dir (str, optional): If provided, not using the default `config.fastcoref_joint.doc_encoder_dir`. Defaults to None.

    Returns:
        _type_: model, subword_tokenizer, max_segment_len
    """
    ...
    # Load the encoder
    model.mention_proposer.doc_encoder.lm_encoder = AutoModel.from_pretrained(
        pretrained_model_name_or_path=doc_encoder_dir
    )
    model.mention_proposer.doc_encoder.tokenizer = AutoTokenizer.from_pretrained(
        pretrained_model_name_or_path=doc_encoder_dir
    )
...
@torch.no_grad()
def inference(model, tokenized_doc, extra_output_dict: dict = None):
    if extra_output_dict is not None:
        pred_mentions, mention_scores, gt_actions, pred_actions = model(tokenized_doc, extra_output=extra_output_dict)
    else:
...
def format_input_tok_same_as_traingset(df_base, spacy_nametyle):
    ...
    while True:
        ...
        for _idx, data in df_sentence.iterrows():
            ...
            token_list2.append(str(conllToken))
...
def run(
    ...
    subword_tokenizer,
    max_segment_len,
    section_docid_extra_output_dict: dict = None,
):
    """
    Args:
        config (_type_): _description_
        use_sections (list): A list of section names to be processed, which are also the folder names.
        model (EntityRankingModel): The fast-coref model loaded from checkpoint
        subword_tokenizer (_type_): _description_
        max_segment_len (_type_): _description_
        section_docid_extra_output_dict (dict, optional): Newly added for active learning.
            Saving the mention logit scores for computing mention entropy. Defaults to None.

    Returns:
        _type_: _description_
    """
    # input csv's namestyle
    spacy_nametyle = config.name_style.spacy.column_name
    # output csv's namestyle
    ...
    for section_name in use_sections:
        ...
        # Newly added to get internal information for active learning.
        section_docid_extra_output_dict[section_name] = {}
        ...
        for file_entry in tqdm(os.scandir(csv_file_dir)):
            ...
            df_base = pd.read_csv(file_entry.path, index_col=0, keep_default_na=False, na_filter=False)
            ...
            tokenized_doc = tokenize_and_segment_doc(
                sent_tok_2d_list, subword_tokenizer, max_segment_len=max_segment_len
            )

            try:
                extra_output_dict = {}
                # Get model output
                pred_mentions, mention_scores, gt_actions, pred_actions = inference(
                    model, tokenized_doc, extra_output_dict
                )

                # Newly added for active learning.
                section_docid_extra_output_dict[section_name][file_entry.name] = extra_output_dict
                ...
                coref_group_aligned_to_input_tok = align_byIndex_individually_nestedgruop(
                    len(input_tok_list), coref_group_list
                )
                coref_group_aligned_to_spacy_tok = align_to_spacy(
                    tok_indices_in_spacy, coref_group_aligned_to_input_tok, input_tok_list, spacy_tok_list
                )

                coref_group_conll_aligned_to_input_tok = align_coref_groups_in_conll_format(
                    len(input_tok_list), coref_group_list
                )
                coref_group_conll_aligned_to_spacy_tok = align_to_spacy(
                    tok_indices_in_spacy, coref_group_conll_aligned_to_input_tok, input_tok_list, spacy_tok_list
                )
                ...
                df_fastcoref_joint = pd.DataFrame(
                    {
                        ...
                        fastcoref_joint_nametyle["coref_group_conll"]: [
                            str(i) for i in coref_group_conll_aligned_to_spacy_tok
                        ],
                    },
                )
