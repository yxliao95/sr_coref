...
def batch_processing(input_cfg, temp_cfg, section_name, input_file_path, keep_0_coref_docs=False) -> int:
    """All whitespces like "\n", "\n " and " " are skipped.
    Return:
        ...
        while True:
            ...
            for _idx, data in df_sentence.iterrows():
                ...
                conllToken = ConllToken(
                    doc_id + "_" + section_name, sentence_id, _idx, data[input_cfg.column_name.token]
                )
...
def convert_to_individual_conll(
    config, input_cfg, temp_cfg, section_name, section_docs_dir, keep_0_coref_docs=False
) -> tuple[Counter, dict]:
    ...
    with ProcessPoolExecutor(max_workers=config.thread.workers) as executor:
        ...
        for file_name in tqdm(doc_files):
            ...
            all_task.append(
                executor.submit(batch_processing, input_cfg, temp_cfg, section_name, input_file_path, keep_0_coref_docs)
            )
...
def prepare_conll(config, input_cfg, temp_cfg, keep_0_coref_docs=False):
    ...
    for section_name in input_cfg.section:
        ...
        doc_coref_counter, corefGroupNum_docId_dict = convert_to_individual_conll(
            config, input_cfg, temp_cfg, section_name, docs_dir, keep_0_coref_docs
        )
        ...
        log_out.append(
            {
                ...
                "File counter": dict(
                    [
                        [f"coref_group_num ({group_num})", f"{doc_num} docs"]
                        for group_num, doc_num in sorted(doc_coref_counter.items(), key=lambda x: x[0])
                    ]
                ),
            }
        )
...
def aggregrate_conll(config, keep_0_coref_docs=False) -> defaultdict[str, int]:
    ...
    for split_cfg in split_cfg_list:
        check_and_remove_dirs(
            os.path.join(config.output.base_dir, config.output.conll_dir_name, split_cfg.dir_name), True
        )
        ...
        if split_cfg.get("sample_detail", None):
            ...
            for sectionName_sampleNum_dict in split_cfg.sample_detail:
                for section_name, sample_num_str in sectionName_sampleNum_dict.items():
                    ...
                    groupNum_allDocId_dict: dict[str, list[str]] = get_actural_doc_ids(
                        os.path.join(config.temp_pred.base_dir, f"{section_name}{config.temp_pred.detail_file_suffix}")
                    )

                    # Get the actual split number according the config.
                    data_split_proportion = [int(i) for i in split_cfg.proportion.split(",")]  # [8, 2]
                    data_split_proportion_norm = [
                        i / sum(data_split_proportion) for i in data_split_proportion
                    ]  # [0.8, 0.2]
                    ...
                    for groupNum, sampleDocNum in ast.literal_eval(sample_num_str).items():
                        ...
                        data_split_num_list = [
                            int(i * sampleDocNum) for i in data_split_proportion_norm
                        ]  # [247.33..., 141.33...]
                        if sampleDocNum == 1:  # Assign to the train set first.
                            data_split_num_list[0] = 1
                        data_split_num_list[-1] = sampleDocNum - sum(
                            data_split_num_list[0:-1]
                        )  # [247, 141] (train, dev)
                        ...
                        docId_testset_list = [
                            ...
                            for i in FILE_CHECKER.filter(
                                os.listdir(os.path.join(split_cfg.test_docs_dir, section_name))
                            )
                        ]
                        ...
                        logger.debug(
                            "len(docId_list): before removing: %s, after removing %s",
                            len(docId_list),
                            len(docId_list_exclude),
                        )

                        for split_name, split_num in zip(data_split_name_list, data_split_num_list):
                            output_dir = os.path.join(
                                config.output.base_dir, config.output.conll_dir_name, split_cfg.dir_name
                            )
                            ...
                            for doc_id in docId_list_shuffle[0:split_num]:
                                input_conll_file = os.path.join(
                                    config.temp_pred.base_dir, section_name, f"{doc_id}{config.output.suffix}"
                                )
            ...
            for split_name in data_split_name_list:
                temp_dict[split_name] = sum(
                    [
                        val_dict[split_name]
                        for _, gNum_dict in log_out[split_cfg.dir_name].items()
                        for _, val_dict in gNum_dict.items()
                    ]
                )
            for split_name in data_split_name_list:
        ...
        # split_cfg such as train_manual_100_1
            ...
            for section_entry in os.scandir(split_cfg.target_doc_dir):
                ...
                groupNum_allDocId_dict: dict[str, list[str]] = get_actural_doc_ids(
                    os.path.join(split_cfg.source_dir, f"{section_name}{config.temp_pred.detail_file_suffix}")
                )
                allDocId_list = [
                    docId
                    for groupNum, docIds in groupNum_allDocId_dict.items()
                    if groupNum != "0" or keep_0_coref_docs
                    for docId in docIds
                ]
                # Remove docs that have 0_coref
                # New: keep 0 coref docs
                docId_trainset_list = [
                    i.rstrip(".csv")
                    for i in FILE_CHECKER.filter(os.listdir(os.path.join(split_cfg.target_doc_dir, section_name)))
                ]
                ...
                for split_name, split_num in zip(data_split_name_list, data_split_num_list):
                    ...
                    for doc_id in docId_list_shuffle[0:split_num]:
                        input_conll_file = os.path.join(
                            split_cfg.source_dir, section_name, f"{doc_id}{config.output.suffix}"
                        )
                        copy_and_paste_conll(input_conll_file, output_conll_file)
        ...
        # split_cfg such as test_gt test_pred
            ...
            log_out[split_cfg.dir_name][split_name] = sum(
                [val_dict[split_name] for _, val_dict in log_out[split_cfg.dir_name].items()]
            )
